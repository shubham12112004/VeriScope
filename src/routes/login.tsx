...
export const Route = createFileRoute("/login")({
  head: () => ({ meta: [{ title: "Login — VeriScope AI" }, { name: "description", content: "Sign in to VeriScope AI." }] }),
  component: LoginPage,
});
    ...
    <AppShell>
      <section className="mx-auto flex max-w-lg px-4 py-16 sm:px-6">
        <form onSubmit={submit} className="veri-panel w-full rounded-[1.75rem] p-6 sm:p-8">
          ...
          <p className="mt-2 max-w-md text-muted-foreground">Access saved analyses, trend dashboards, and admin review queues with JWT-backed sessions.</p>
          ...
          <Button className="mt-5 w-full" variant="hero" disabled={loading}>{loading && <Loader2 className="animate-spin" />} Sign in</Button>
          <p className="mt-4 text-center text-sm text-muted-foreground">
            No account? <Link className="font-semibold text-primary hover:underline" to="/register">Create one</Link>
          </p>
        </form>
      </section>
    </AppShell>
